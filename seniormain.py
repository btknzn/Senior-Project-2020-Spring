...
def System():
    ...
    while(True):
        ...
        yourTime = round(time.time()- starttime)    
        cv2.putText(frame,"Goz Kirpma Sayisi: "+ str(counter),(0,200),font,0.6,(255,0,0),1)    
        if ((starttime+10)<time.time()):
            if(situation==False):
                situation = True
                if ( counter == 0):
                    im = cv2.imread('hıcbırsey.jpg')                    
                if (counter == 1):
                    im = cv2.imread('yatak.jpg')
                if (counter == 3):
                    im = cv2.imread('sitting.jpg')
                if (counter == 4):
                    im = cv2.imread('WC.jpg') 
                if (counter == 5):
                    im = cv2.imread('travelling.jpg') 
                if (counter == 6):
                    im = cv2.imread('colfotograf.jpg')
                if (counter == 7):
                    im = cv2.imread('dag.jpg') 
                if (counter == 8):
                    im = cv2.imread('gece.jpg')
                if (counter == 9):
                    im = cv2.imread('aydınlık.jpg')
                if (counter == 2):
                    im = cv2.imread('film.jpg')
                if (counter == 10):
                    im = cv2.imread('yemek.jpg') 
                
                print('Sizin isteğiniz budur  '+systemTupel[counter] )
                im = cv2.resize(im,(320,320))
                cv2.putText(im,systemTupel[counter],(0,200),font,2,(0,255,0),2)
                winname = "FRAME"
        ...
        cv2.namedWindow(winname)
        cv2.moveWindow(winname,0,0)
        cv2.putText(frame,"Zaman: "+str(yourTime)+"saniye",(0,100),font,0.6,(0,0,0),2)
...
cap.set(3,480)
cap.set(4,320)
systemTupel = ("Hicbirsey","Yatmak","Film","Oturmak","Tuvalet","Gezinmek","Sicak","Soguk","Karanlik","Aydinlik","yemek","","","","","","","","","","")
